...
> directives changed:
> - `style-src` extended from `'self' 'unsafe-inline'` to `'self'
>   'unsafe-inline' https://accounts.google.com`. Reason: GIS loads
>   `https://accounts.google.com/gsi/style` for its sign-in button; the
>   prior policy blocked it, leaving the button unstyled.
> - `trusted-types` extended from `default` to `default goog#html`.
>   Reason: GIS internally registers a Trusted Types policy named
>   `goog#html` to safely sink HTML; the prior policy blocked the
>   registration and broke GIS internals (sign-in popup never rendered
>   beyond the button frame).
>
> Evidence: live-deploy puppeteer smoke-test on 2026-05-01 captured
> three blocking console errors from `accounts.google.com/gsi/client`
> matching the relaxations above; the prior CSP could not have worked
> with GIS without these allowances. The relaxation is the **minimum
> surface** required to keep §8 auth functional and does NOT widen
> script-src, connect-src, or frame-src (which already permit
> accounts.google.com per v1.0.0). Per §3.5 / §3.7 the change is
> mirrored in `nginx.conf` byte-for-byte.
...
| 3.4  | (v1.4.0 amendment) The CSP must include all of: `default-src 'self'`, `style-src 'self' 'unsafe-inline' https://accounts.google.com`, `connect-src 'self' https://lrclib.net https://api.mymemory.translated.net https://accounts.google.com https://oauth2.googleapis.com`, `img-src 'self' blob: data: https://lh3.googleusercontent.com https://*.googleusercontent.com`, `media-src 'self' blob: data:`, `font-src 'self'`, `frame-src https://accounts.google.com`, `worker-src 'self'`, `manifest-src 'self'`, `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, `frame-ancestors 'none'`, `upgrade-insecure-requests`, `require-trusted-types-for 'script'`, `trusted-types default goog#html`. Adding a new directive or relaxing an existing one requires a security review note in the PR. |
| 3.5  | Removing or weakening any directive in §3.4 is a **doctrine amendment**, not a routine change.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 3.6  | (v1.4.0 amendment) A Trusted Types `default` policy must be installed by the first inline script in `<head>`. `createScript` must throw. `createScriptURL` must allow only same-origin and `https://accounts.google.com`. `createHTML` is pass-through because all template sinks escape user input at the boundary (see §4). The CSP `trusted-types` directive additionally permits the `goog#html` policy registered internally by the Google Identity Services SDK loaded from `accounts.google.com` — this policy is owned by Google, not us, and is required for the GIS sign-in button to function (see v1.4.0 amendment note). We do not register or rely on `goog#html` from our own code. |
