/**
 * LESSON V2 ENGINE - Knowledge AIO
 * ================================
 *
 ...
 * deterministic.
 *
 * Registered as the `lessonv2` layout in TopicLessonLayouts and pinned to a
 * topic via TOPIC_DEFAULT_LAYOUT (cybersecurity today). It is out of the
 * rotation, so no other domain meets it by chance.
 *
...
/**
 * Doctrine §43.2 — registry metadata. Out of the rotation (`rotate: false` in
 * TopicLessonLayouts): a topic opts into this loop rather than meeting it by
 * chance, because it is a long-form arc rather than a variation.
 */
export const LAYOUT_META = {
  ...
  icon: '🧪',
  description: 'Chunked teach-then-practice with interleaving and a terminal lab',
  descriptionIt: 'Impara-e-pratica a blocchi, con interleaving e un lab da terminale',
};

/**
 * Doctrine §43.4 — the loop splits the lesson into two chunks and drills each
 * before interleaving them, so it needs enough translatable items to fill both.
 * Below that the registry falls through to the rotating layouts.
 */
export function canRender(lesson) {
  const items = lesson?.items;
  if (!Array.isArray(items)) return false;
  return items.filter((it) => it && it.english && it.italian).length >= 4;
}
...
export class LessonV2Engine {
  ...
  /** Entry point — called via TopicLessonLayouts.createLessonEngine(). */
