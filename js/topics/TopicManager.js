...
import { topicPlacementMixin } from './TopicPlacement.js';
  ...
  renderLessonItem() {
    ...
    container.innerHTML = `
      <div class="lesson-item-card topic-item-card">
        <div class="item-pair">
          <div class="item-english">${escapeHtml(item.english)} ${ttsBtn}</div>
          <div class="item-separator"></div>
          <div class="item-italian">${escapeHtml(item.italian)}</div>
        </div>
        <div class="item-pronunciation">
          Pronuncia: <strong>${escapeHtml(item.pronunciation)}</strong>${item.phonetic ? ` (${escapeHtml(item.phonetic)})` : ''}
        </div>
        ${item.example ? `<div class="item-example">\u{1F4DD} "${escapeHtml(item.example)}"</div>` : ''}
        ${enrichmentHtml}
      </div>
  ...
  completeLesson() {
    ...
    container.innerHTML = `
      <div class="lesson-completion">
        ...
        <p class="completion-detail">${escapeHtml(this.currentLesson.title)}</p>
